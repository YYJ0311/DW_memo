```
extends 기준으로 왼쪽은 자식클래스(서브클래스), 오른쪽은 부모클래스(슈퍼클래스)
...
오버로딩과 오버라이딩
    오버로딩 
        같은 클래스 or 상속 관계에서 동일한 이름의 메소드 중복 가능
        조건
            메소드 이름 동일, 파라미터(인수, 인자, 매개변수) 개수와 타입은 다르게 사용
        사용하는 이유
            이름이 같은 여러 개의 메소드를 선언하기 위해

    @Override(오버라이딩)
        자식 클래스(서브 클래스)에서 부모 클래스(슈퍼 클래스)에 있는 메소드를 동일한 이름의 메소드로 재정의
        조건
            메소드 이름은 동일, 파라미터(인수, 인자, 매개변수) 개수와 타입도 같게 사용
        사용하는 이유
            부모 클래스 메소드를 무시하고 자식 클래스에서 재정의하기 위해

    오버로딩은 헷갈려서 가끔 쓰고, 오버라이딩은 매우 많이 쓴다!
    객체지향언어의 특징임
```
