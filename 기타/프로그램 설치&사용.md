...
# 이클립스
    ...
    이클립스로 깃허브 사용하기
        ...
        오류
            푸쉬 전 로그인 안 되는 오류 발생
            => 1. 깃허브 웹 페이지에서 토큰 새로 발급받음
                2. 이클립스 Git Repositories > Remotes > origin > 주소 우클릭 - Change Credentials - 깃허브 로그인 아이디와 새로 발급 받은 토큰을 넣고 Save
                3. 이어서 push 하기
    ...
    이클립스 롬복(lombok) 적용하기
        ...
        lombok 간단한 install
            ...
            (* 설치 위치를 eclipse나 sts.exe가 위치하는 폴더 내로 설정해야 함)
            이클립스에서 프로젝트 clean/프로젝트 update - 재시작
    ...
    파일 여는 내부/외부 프로그램 설정
        Window - Preferences - General - Editors - File Associations에서 file types 추가하고, assocated editors로 열기위한 프로그램 설정함
        
        ex) 프로젝트 내부의 .sql 파일을 외부 프로그램으로 열 수 없어서 내부 프로그램 중에 text editor로 열어서 확인함
...
# Git
    - 명령어
        ...
        git push origin 브랜치명 : 브랜치에 올리기
            * remote 저장소에 해당 브랜치가 존재하지 않으면 새로 생성됨
        git push origin -d 리모트브랜치명 : 리모트 브랜치 삭제
        git branch : local 저장소 branch 확인
        git branch -r : remote 저장소 branch 확인 
        git branch -al : local, remote 모두에 존재하는 branch 확인
        git checkout 브랜치명 : 관리할 브랜치 바꾸기
        ...
        * pull을 하지 않고 작업한 뒤 push할 경우 꼬이지 않게 에러메세지 출력해주는 명령어
        ...
        git reset {커밋해쉬} : 특정 커밋 상태로 돌아가는 명령어
            커밋 해쉬는 깃허브 웹페이지나, 배쉬창에서 git log로 확인 가능(q로 나감)
            reset의 종류
                1) reset : 입력한 커밋 이후의 변경사항은 모두 unstaged 영역에 남김
                2) reset --soft : staged 영역에 남김
                3) reset --hard : 변경사항 모두 제거(로컬에서 모두 사라지므로 신중하게 사용!)
            작업 중 이슈사항
                일부 파일을 삭제하고 commit 후 push까지 한 상태에서 삭제한 파일을 복구하고 싶음
                1번의 일반 reset을 사용하여 삭제하기 이전 커밋으로 돌아갔지만 파일이 복구되지 않음
                파일 복구는 수동으로 수행하여, 다른 수정된 부분과 함께 commit 하고 push
                    이슈 발생 : 깃허브에 이미 올라간 commit 부분을 지우고 새로 commit 한 것이기에 충돌을 일으킴
                    해결 : git push -f origin 브랜치명 으로 강제(force) push를 수행함
            그 결과 이전에 commit한 기록은 지워지고 새로 commit한 내용이 기록됨
    ...
    - gitignore
        ...
        gitignore 적용해서 푸쉬하기
            로컬에서 브랜치 새로 생성 및 이동
            캐쉬 삭제 후 add, commit 하고 별도 브랜치로 push
            이상 없음 확인하고 master로 pr
    
    - 강제 branch merge
        https://opendive.tistory.com/691

    - local 프로젝트에서 새로운 브랜치 생성 후 remote 저장소에 반영 및 연결하기
        프로젝트 폴더 내에서 시작
        git init
        git remote add origin 저장소주소
        git add .
        ...
        branch를 생성 또는 이름 변경
        git checkout 브랜치명
        git push origin 브랜치명 : 여기서 remote 저장소에 해당 브랜치가 존재하지 않으면 새로 생성됨
        * git branch --set-upstream-to origin/브랜치명 : local과 remote의 branch를 연결
        
        이후 두 저장소 간 git pull, push 사용 가능
    
    - 로컬 저장소에서 원격 브랜치 변경하여 연결하고 push하기
        원격 저장소에서 브랜치를 생성한 경우 로컬에서 git fetch 수행
        로컬 저장소의 브랜치명 변경
            git branch로 로컬의 브랜치명 확인
            git branch -m 변경대상브랜치 변경할브랜치명
        원격 저장소의 특정 브랜치로 연결
            git branch -u origin/원격브랜치명 로컬브랜치명
        연결 확인
            git branch -vv을 이용해서 로컬 브랜치명과 연결된 원격 브랜치명을 확인가능하다.

    - 폴더 내 git 사용자이름과 이메일 정보 삭제하기
        이름과 이메일 확인
    ...
    - 오류
        ...
        3. 브랜치명을 변경했다던가 특정 이유로 git push 또는 pull에서 다음의 문구와 함께 실행되지 않는 경우
            "refusing to merge unrelated histories"
            해결방법
                git pull origin 브랜치 명 --allow-unrelated-histories
                => 서로 관련 기록이 없는 두 프로젝트를 병합하는 것을 허용해 줌
                이후 git push --set-upstream origin main
        
        4. Not a valid object name: 'master'
            local에 있는 프로젝트에 새 branch를 만드려고 할 때 발생함
            해결방법
                아직 commit을 한 번도 하지 않은 repository이라서 발생한 오류로, git add . 와 git commit - m "메세지" 입력 후 브랜치 생성이 가능하다.
...
# jupyter notebook
    ...
    - 시작폴더 변경
        anaconda prompt 실행 -> jupyter notebook --generate-config 입력하여 config 파일 생성 및 경로 확인
        해당 경로의 파일을 메모장으로 열기 -> notebook_dir 항목을 찾아서 원하는 경로 입력하고 맨 앞의 # 제거
        jupyter notebook 바로가기 아이콘 속성 ->  "%USERPROFILE%/" 와 %HOMEPATH% 제거