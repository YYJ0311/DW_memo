...
# 추상화(Abstract)
	...
	추상 메소드(abstract method)
		...
		모듈처럼 중복되는 부분이나 공통적인 부분은 미리 다 만들어진 것을 사용하고, 이를 받아 사용하는 쪽에서는 자신에게 필요한 부분만을 재정의하여 사용함으로써 생산성이 향상되고 배포등이 쉬워진다.
		사용 목적
			추상 메소드가 포함된 클래스를 상속받는 "자식 클래스"가 "반드시 추상 메소드를 구현"하도록 하기 위함
	
	추상 클래스(abstract class)
		하나 이상의 추상 메소드를 포함하여 abstract 키워드로 선언된 클래스
...
# 인터페이스(interface)
	...
	클래스를 이용하여 다중 상속을 할 경우 메소드 출처의 모호성 등의 문제가 발생할 수 있기 때문에 자바에서는 클래스를 통한 다중 상속은 지원하지 않는다.
	대신 인터페이스를 implements하여 다중상속을 지원하고 있다.

	다른 클래스를 작성할 때 기본이 되는 틀을 제공하면서, 다른 클래스 사이의 중간 매개 역할까지 담당하는 일종의 추상 클래스이다.
	추상 클래스가 추상메소드 뿐만 아니라 생성자, 필드, 일반 메소드까지 포함할 수 있는 것과 반대로,
	...
	장점
		...
		4. 클래스와 클래스 간의 관계를 인터페이스로 연결하면, 클래스마다 독립적인 프로그래밍이 가능하다.. - 아직 잘 모르겠음
...
# interface와 abstract 차이
	존재 목적의 차이
		추상(클래스) : 추상클래스를 상속받아서 기능을 이용하고 확장시킴
		인터페이스 : 함수의 껍데기만 가지고 있으면서 상속받은 객체들에 함수의 구현을 강제하고, 같은 동작을 보장하기 위함
	
	세부적인 차이점
		추상 클래스
			추상 메소드를 반드시 1개 이상 포함
			일반 메소드, 일반 변수, 생성자 포함 가능
			추상 클래스 간 상속 가능(오버라이드 안 해도 됨)
			접근지정자(public / private / protected / default) 모두 가능
			상속 시 extends 사용
			인터페이스보다 속도가 더 빠르다. 인터페이스는 관련 메소드를 찾기 위해 부가적인 일을 더 처리함.

		인터페이스
			클래스가 아님
			비어있는 추상 메소드만 포함
			메소드에 final 사용불가. 변수는 모두 static이어야 함.
			일반 메소드, 일반 변수, 생성자 포함 불가능
			접근지정자는 없거나 public abstract만 가능
			인터페이스간 상속 가능(내용이 없는 메소드를 상속받는 것으로 구현이 아니므로 extends을 사용함)
			클래스가 아니므로 상속시 implements를 사용한다
			변수 앞에 static final이나 메소드 앞에 abstract를 쓰지 않아도 자동인식한다

	public abstract interface TestMapper; 
    	프로젝트 패키지를 보면 mapper interface에다 abstract를 같이 붙여서 사용하고 있다.
    	하지만 인터페이스는 이미 추상(abstract)을 지원하기 때문에 중복해서 abstract를 적어지 않아도 된다.
		또 interface는 public만 가능하므로 생략하고 interface TestMapper; 라고 써도 된다.
