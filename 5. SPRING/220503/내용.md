...
패키지가 3개인 이유
	- 첫번째 패키지에 com, io, kr 등, 두번째는 회사이름 또는 조직이름, 세번째는 프로젝트명으로 패키지를 나눠서 알아보기 쉽게 만든다.
	
controller의 의미
	- url을 요청받는 곳으로, 특정 주소에 접속할 때 응답을 해주는 부분임.
...
public class ActorController {
	...
	public List<UserVO> callUserList(HttpServletRequest request){
        // controller의 함수 이름은 call 또는 load로 한다.
...
@Controller
    ...
    2. @Autowired 객체 생성과 소멸을 Spring이 관리해 줌
        
@Service
    비즈니스 로직, 계산 수행, 외부 API 호출을 수행하는 Java 클래스를 표시
어노테이션을 붙여서 Spring에 알려준다.

어떤 ip가 접속을 했는지도 controller에서 확인할 수 있다.
    어떤 지역에서 들어왔는지 통계낼 수 있음

http 안에 header와 body가 들어있다. 그리고 header에 ip주소가 들어있다.
    => request.getHeader("X-forwarded-For")로 ip를 불러올 수 있다.
    만약 여기에 ip가 없다면 request.getRemoteAddr()로 ip를 갖고온다.
```

```
실무에서 생성자는 많이 쓰지 않음
    Spring에 연결된 DB가 알아서 VO의 setter함수를 찾아서 데이터를 set해줌
```